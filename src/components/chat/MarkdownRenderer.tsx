          ...
          <div key={currentIndex++} className="my-4">
            <div className="bg-gradient-to-br from-primary/10 to-accent/5 border border-primary/20 rounded-lg overflow-hidden shadow-lg backdrop-blur-sm">
              ...
              <pre className="p-6 overflow-x-auto max-w-full">
                <code className="text-sm font-mono text-foreground leading-relaxed break-words whitespace-pre-wrap">
                  {codeLines.join("\n")}
                </code>
              </pre>
            </div>
                ...
                {dataRows.map((row, rowIdx) => (
                  <tr key={rowIdx} className="hover:bg-muted/50 transition-colors">
                    {row.map((cell, cellIdx) => (
                       <td
                         key={cellIdx}
                         className="px-6 py-4 text-sm text-foreground border-r border-border last:border-r-0 break-words max-w-xs"
                       >
